...
public class DateAndTimeUtil {
     ...
     */
    public static String checkOption(String option, String tempDate) {
        ...
        cl.setTime(date);
        if (PRE_STRING.equals(option)) {
            // 时间减一天
            cl.add(Calendar.DAY_OF_MONTH, -1);

        } else if (NEXT_STRING.equals(option)) {
            // 时间加一天
    ...
    public static String[] getNowPreDate(String date) throws ParseException {
        ...
        if (date.equals(stMonth)) {
            ...
            endate = sdfFull.format(now);// 2014-11-24
            preDateStart = subMonth(stdate,1);// 2014-10-01
            preDateEnd = subMonth(endate,1);// 2014-10-24
        } else {
            // 非当前月
            String monthMaxDay = getMonthMaxDay(date);
            stdate = date + "-01";// 2014-10-01
            endate = date + "-" + monthMaxDay;// 2014-10-31
            preDateStart = subMonth(stdate,1);// 2014-09-01
            preDateEnd = subMonth(endate,1);// 2014-09-30
        }
        strDate[0] = stdate;
        strDate[1] = endate;
